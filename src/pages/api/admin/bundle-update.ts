...
import { updateBundle } from '../../../lib/admin/bundles';

const BundleUpdate = z.object({
  id: z.string().uuid(),
...
export const POST: APIRoute = async (ctx) => {
  ...
  const parsed = BundleUpdate.safeParse(Object.fromEntries(entries));
  if (!parsed.success)
    return ctx.redirect(`/admin/bundles?error=${encodeURIComponent('Invalid input')}`, 303);
